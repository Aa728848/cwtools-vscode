...
- **Paradox Override Strategy (Block-level LIOS / FIOS)**: Override in Paradox engines is definition/key-scoped (block-level), NOT whole-file. Untouched vanilla definitions remain active automatically. Under LIOS (e.g. \`common/scripted_triggers/\`), write ONLY the specific modified key(s) in your own mod file; NEVER copy the entire vanilla file or duplicate untouched vanilla keys. Use \`query_override_modes\` to confirm the strategy before creating files.
