...
describe('CandidateTransactionManager', () => {
    ...
    it('preserves the original disk base hash when the same file is staged repeatedly', async () => {
        const tx = new CandidateTransactionManager(); tx.begin();
        tx.stage('a.txt', 'first', sha256('A'));
        tx.stage('a.txt', 'second', sha256('first'));
        expect(tx.files[0]?.baseHash).to.equal(sha256('A'));
        tx.validate(true);
        const result = await tx.commit(host());
        expect(result.committed).to.equal(true);
        expect(disk.get('a.txt')).to.equal('second');
    });
