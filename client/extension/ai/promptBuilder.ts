...
- **RUN COMMAND CWD**: \`run_command\` defaults to the Project Workspace Root, not the scratch directory. Keep the default cwd for project scripts unless the user asks otherwise. **Preferred approach for running scratch scripts**: use the \`.cwtools-ai/{current-topic-id}/scratch/agent_helper.py\` alias directly — the system will resolve it to the correct absolute path automatically: \`python ".cwtools-ai/{topic-id}/scratch/my_script.py"\`. **Shell-specific environment variable syntax**: In **non-Utility mode** (cmd.exe), reference env vars as \`%CWT_AGENT_SCRATCH_DIR%\`; in **Utility mode** (PowerShell), use \`$env:CWT_AGENT_SCRATCH_DIR\`. NEVER mix syntaxes (e.g. do NOT use \`$env:\` in cmd.exe or \`%VAR%\` in PowerShell). **Paths with spaces**: Always wrap paths containing spaces in double quotes: \`python "%CWT_AGENT_SCRATCH_DIR%\\script.py"\` (cmd) or \`python "$env:CWT_AGENT_SCRATCH_DIR\\script.py"\` (pwsh). The \`.cwtools-ai/...\` alias approach is strongly preferred over env-var concatenation because the system handles quoting automatically.
...
- For temporary command-support Python scripts, reuse and overwrite one script for the whole task: \`CWT_AGENT_HELPER_SCRIPT\` / \`.cwtools-ai/{current-topic-id}/scratch/agent_helper.py\`. Put search, replace, and verify modes in that one helper instead of creating multiple scratch scripts. Delete the helper only when it is a temporary execution/verification helper and the batch/verification step is finished; preserve user-requested deliverable scripts, scripts the user explicitly asked you to create, and existing project scripts. **Preferred way to run scratch scripts in Utility Mode (PowerShell)**: use the \`.cwtools-ai/{topic-id}/scratch/agent_helper.py\` alias — the system resolves it automatically, e.g. \`python ".cwtools-ai/{topic-id}/scratch/agent_helper.py"\`. When you must reference env vars in PowerShell, use \`$env:CWT_AGENT_SCRATCH_DIR\` (NOT \`%VAR%\` cmd syntax). Always wrap paths containing spaces in double quotes.
