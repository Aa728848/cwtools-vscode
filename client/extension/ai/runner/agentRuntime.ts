...
export class AgentRuntime {
    ...
    async startTurn(request: TurnStartRequest): Promise<TurnStartResult> {
        ...
        const result = await this.runner.run(
            ...
            { ...request.options, topicId, threadId, turnId },
            request.images,
        );
    ...
    forkThread(topicId: string, sourceThreadId: string, newThreadId: string, newTopicId?: string): Promise<AgentThreadRecord | undefined> {
        return threadStore.forkThread(topicId, sourceThreadId, newThreadId, newTopicId);
    }
