...
`manifest.json` 只保存 freshness、指纹、计数、领域和可移植数据库引用；规范化事实只在 `knowledge.sqlite` 中保存一次。达到定义预算时，选择器会完整保留工作区定义，并依据 CWTools 报告的定义类型与引用拓扑分配剩余原版预算，不再优待架构内写死的游戏概念，也不按字母序直接截断。拓扑达到上限时会明确发布为 `partial`，不再宣称 `ready`。V1 迁移只有在新数据库和 manifest 都发布成功后才清理旧的 capability/archetype/snapshot JSON。文件 watcher 通过绝对变更文件身份刷新定义与拓扑，不再根据目录名猜测 TypeDef；组合式多根工作区知识包会把每个根的变更路由到所属 manifest，并将全部项目根纳入指纹。配置变化仍执行完整重建。原版指纹同时包含当前游戏的序列化 `.cwb`；当 `.cwb` 变化时，同一刷新阶段会先重建对应的全局原版符号 SQLite，再执行完整的 `knowledge.sqlite` 导出。`cwtools.ai.queryProjectKnowledgeDb` 是 Extension 与 MCP 共用的只读查询命令，调用方和 LSP 都会拒绝项目边界之外的数据库路径。显式标识符会先通过定义索引解析为种子，再沿通用的入站/出站 typed reference 扩展；请求的实体类型也会约束初始入站引用切片。只有纯意图查询才退回有界文本检索。Agent 通过 `query_project_knowledge` 按任务检索项目/原版引用证据；复杂蓝图仍必须完成活动 CWT/LSP 合法性验证，并确保关键未解决列表为空。
