...
export function preflightCommand(commandLine: string): CommandPreflightResult {
    ...
    for (const raw of rawSegments) {
        ...
        if (DESTRUCTIVE_COMMANDS.has(baseCmd) || DESTRUCTIVE_COMMANDS.has(trimmed.toLowerCase()) || trimmed.toLowerCase().includes('rm -rf') || trimmed.toLowerCase().includes('del /s')) {
            ...
            riskLevel = Math.max(riskLevel, 2) as any;
            requiresPermission = true;
        } else if (hasRedirect || WRITE_COMMANDS.has(baseCmd) || WRITE_COMMANDS.has(secondCmd)) {
