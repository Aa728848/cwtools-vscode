...
export function formatEventPayload(event: any, i18n?: ChatI18nText): string {
    ...
    const tt = (en: string, zh: string) => i18n?.locale === 'zh-cn' ? zh : en;
    ...
    if (event.type === 'cache_stats') {
        ...
        html += `<h4>${svgIcon('sparkles')} ${tt('Prefix Cache Efficiency Audit', '前缀缓存效能审计')}</h4>`;
        
        // 三柱图
        html += `<div class="inspector-section">`;
        html += `<strong>${tt('Cache distribution:', '缓存效能分布:')}</strong>`;
        html += `<div class="cache-sparkline" style="max-width: 100%; height: 10px; margin: 8px 0;" title="${tt('Cache hit', '缓存命中')}: ${cached} | ${tt('Cache creation', '缓存新建')}: ${created} | ${tt('Miss/uncached', '穿透/未命中')}: ${miss}">`;
        ...
        // 细节列表
        html += `<div class="inspector-section">`;
        html += `<strong>${tt('Details:', '详细统计:')}</strong>`;
        html += `<ul>`;
        html += `<li><strong>${tt('Cache hit (Hit):', '缓存命中 (Hit):')}</strong> <code style="color:var(--vscode-charts-green); font-weight:bold;">${cached} tokens</code> (${cachedPct.toFixed(1)}%)</li>`;
        html += `<li><strong>${tt('Cache creation (Create):', '缓存新建 (Create):')}</strong> <code style="color:var(--vscode-charts-blue); font-weight:bold;">${created} tokens</code> (${createdPct.toFixed(1)}%)</li>`;
        html += `<li><strong>${tt('Cache miss (Miss):', '缓存穿透 (Miss):')}</strong> <code style="color:var(--vscode-charts-orange); font-weight:bold;">${miss} tokens</code> (${missPct.toFixed(1)}%)</li>`;
        html += `<li><strong>${tt('Total context size:', '总上下文大小:')}</strong> <code>${total} tokens</code></li>`;
        html += `<li><strong>${tt('Estimated cost saved for this inference:', '本次推断成本节省:')}</strong> <span class="cache-savings" style="color:var(--vscode-charts-green); font-weight:bold;">¥${(cs.savedCostCny || 0).toFixed(6)}</span></li>`;
        html += `</ul>`;
        html += `</div>`;

        // 原生日志内容
        html += `<div class="inspector-section"><strong>${tt('Audit log:', '审计日志:')}</strong><pre>${escapeHtml(event.content || '')}</pre></div>`;
        html += `</div>`;
        return html;
    }
