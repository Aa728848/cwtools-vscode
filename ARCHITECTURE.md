...
- 工作区符号持久化到 `.cwtools/index/workspace-symbols.sqlite`；原版符号按原版根目录分库存入 extension global storage。构建时先恢复缓存行，再按文件 `size + mtime` 只解析变化文件，并使用有限并发、排序名称数组和二分前缀区间。普通查询仍保持懒加载，而 `/init` 会在深层知识导出前直接创建工作区数据库。`.cwb` 与元数据成功重写后会发送经过校验的 `vanillaCacheGenerated` 通知；Extension 在窗口重载前强制重建匹配的原版数据库，并把同时到达的同游戏 watcher 刷新合并进这次构建。
...
`manifest.json` 只保存 freshness、指纹、计数、领域和可移植数据库引用；规范化事实只在 `knowledge.sqlite` 中保存一次。`/init` 会请求 `completeExport`，完整保留已加载的全部定义，并在不套用有界快照上限的情况下枚举所有工作区拓扑引用。拓扑收集器在单遍处理中直接去重，不再额外物化第二份 distinct 集合。内部有界调用仍保留明确上限，达到上限时发布 `partial` 而不是 `ready`；确定性的 `partial` 只发布一次，不再重复相同导出，临时的 `loading`/`stale` 仍可重试。V1 迁移只有在新数据库和 manifest 都发布成功后才清理旧的 capability/archetype/snapshot JSON。文件 watcher 通过绝对变更文件身份刷新定义与拓扑，不再根据目录名猜测 TypeDef；组合式多根工作区知识包会把每个根的变更路由到所属 manifest，并将全部项目根纳入指纹。配置变化仍执行完整重建。原版指纹同时包含当前游戏的序列化 `.cwb`；其文件系统 watcher 继续作为兜底，并把 stale 原因持久写入匹配项目的 manifest。由于缓存发布需要重载窗口，重载后的 Extension 只在收到 `cwtools/serverReady` 后恢复完整的 `knowledge.sqlite` 刷新，不会重复已经完成的原版符号构建。`cwtools.ai.queryProjectKnowledgeDb` 是 Extension 与 MCP 共用的只读查询命令，调用方和 LSP 都会拒绝项目边界之外的数据库路径。显式标识符会先通过定义索引解析为种子，再沿通用的入站/出站 typed reference 扩展；请求的实体类型也会约束初始入站引用切片。只有纯意图查询才退回有界文本检索。Agent 通过 `query_project_knowledge` 按任务检索项目/原版引用证据；复杂蓝图仍必须完成活动 CWT/LSP 合法性验证，并确保关键未解决列表为空。
