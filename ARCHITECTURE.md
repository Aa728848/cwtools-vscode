...
Every non-Workflow turn with automatic intent selection asks the configured model for a semantic intent, strategy, confidence, bounded evidence, and whether a material choice still belongs to the user. The capability domain is always selected by the user and is never model-routed. `agentProfile.ts` validates the response, enforces explicit no-write constraints, keeps unresolved user decisions out of Execute mode, and falls back to deterministic admission when routing is unavailable. The Webview shows a transient routing status and a bounded, expandable decision summary rather than hidden chain-of-thought. The router may recommend a strategy but cannot automatically commit an ordinary broad task to Multi-Agent; explicit delegation is retained, while all other fan-out is decided later from a validated task graph. Routing never changes the user-owned permission profile or approval policy.
...
Recovery is typed and bounded at two levels. `RecoveryCoordinator` classifies cancellation, context overflow, rate limits, transport faults, sandbox availability, permission denial, tool validation, and provider failures without flattening their causes; each class owns an independent retry budget. `RecoveryStormBudget` aggregates repeated child failures across a task graph, stops further mutating work when a threshold spans enough nodes, and still permits safe read-only diagnostics. Resume checkpoints persist validated retry `StepRequest` payloads instead of an untyped side channel. The private `history` tool searches only bounded persisted transcripts, honors history persistence policy, excludes tool/system output by default, redacts local paths, and treats results as untrusted evidence.

Provider-native behavior is capability-gated by the effective adapter and endpoint. Official DeepSeek Chat Completions can use at most two Beta prefix-continuation calls for pure text/reasoning responses truncated by length; custom relays, tool-call turns, and other protocols stay on the ordinary Runner continuation path. Usage from all continuation calls is merged. Persistent memory serializes writes per topic, exposes per-key optimistic revisions, supports recoverable archive or explicit deletion, and records a bounded metadata-only recall trace. A loaded skill's `allowed-tools` becomes an enforced per-run upper bound. Trusted-workspace command hooks use validated, bounded `.cwtools/hooks.json` input, allowlisted VS Code commands, configurable timeouts, and explicit ignore/block failure policy; hooks never expand Registry, scheduling, sandbox, or permission authority.
...
每个启用自动意图选择的非 Workflow 回合都会请求当前模型按语义判断意图、策略、置信度、有界证据，以及是否仍存在必须由用户决定的关键选择。能力领域始终由用户手动选择，模型路由绝不切换领域。`agentProfile.ts` 会验证结果、强制遵守显式只读约束、禁止存在未决用户选择的请求进入执行模式，并在路由不可用时回退到确定性准入。Webview 会显示临时路由状态和可展开的有界判断摘要，而不是隐藏思维链。路由器可以提出策略建议，但普通宽任务不会再在入口自动固定为多 Agent；用户显式委派会被保留，其余 Fan-out 均在取得并验证任务图后决定。路由不会改变始终由用户控制的权限 Profile 或审批策略。
...
恢复机制在两个层级上保持类型化和有界。`RecoveryCoordinator` 区分取消、上下文溢出、限流、传输故障、沙箱不可用、权限拒绝、工具校验和 Provider 故障，不会抹平原始原因，各类别使用独立重试预算。`RecoveryStormBudget` 跨任务图聚合子任务的重复失败；当失败达到节点数与次数阈值时停止后续修改型工作，同时仍允许安全的只读诊断。Resume Checkpoint 持久化经过校验的 retry `StepRequest`，不再依赖无类型旁路。私有 `history` 工具只检索有界持久化 Transcript，遵守历史持久化策略，默认排除 tool/system 输出并脱敏本地路径；其结果始终是不受信任的辅助证据。

Provider 原生行为按实际 Adapter 与 Endpoint 能力启用。只有官方 DeepSeek Chat Completions 的纯文本/推理结果因长度截断时，才会最多执行两次 Beta prefix continuation；自定义网关、工具调用回合和其他协议继续使用普通 Runner 路径，所有续写请求的用量合并统计。持久记忆按 Topic 串行写入，提供逐 Key 乐观版本、可恢复归档、显式删除和有界的纯元数据召回轨迹。Skill 的 `allowed-tools` 会成为当前 Run 的真实执行上限。受信任工作区的命令 Hook 使用经过校验且有界的 `.cwtools/hooks.json`、VS Code 命令白名单、可配置超时和明确的 ignore/block 失败策略；Hook 不能扩大 Registry、调度、沙箱或权限授权。
