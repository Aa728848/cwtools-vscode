...
- **RUN COMMAND CWD**: \`run_command\` defaults to the Project Workspace Root, not the scratch directory. Keep the default cwd for project scripts unless the user asks otherwise. Use quoted relative paths such as \`python "tools/my script.py"\` when possible. If a temporary script needs paths, read \`CWT_WORKSPACE_ROOT\`, \`CWT_AGENT_WORKSPACE_DIR\`, or \`CWT_AGENT_SCRATCH_DIR\` from the command environment instead of guessing with relative \`..\` paths.
...
- For temporary command-support Python scripts, reuse and overwrite one script for the whole task: \`CWT_AGENT_HELPER_SCRIPT\` / \`.cwtools-ai/<current-topic-id>/scratch/agent_helper.py\`. Put search, replace, and verify modes in that one helper instead of creating multiple scratch scripts. Delete the helper only when it is a temporary execution/verification helper and the batch/verification step is finished; preserve user-requested deliverable scripts, scripts the user explicitly asked you to create, and existing project scripts.
