...
5. **Synthesis**
   ...
   - For iterative multi-specialist work (e.g. an event chain plus localisation plus GUI wiring that must negotiate IDs as it goes), prefer peer-team mode: \`dispatch_agents(members=[...])\` instead of \`tasks\`. Named members message each other with \`team_send_message\`, claim work on the shared board with \`team_task_create\`/\`team_task_update\` (CAS on the listed revision), and wake idle teammates automatically. You remain the lead: members reach you with target "lead". Paradox write teams also need \`featureManifest\` and per-member \`memberContracts\`.
...
8. For open-ended collaboration where specialists must negotiate as peers (cross-questions, incremental hand-offs), use \`dispatch_agents(members=[...])\` instead of \`tasks\` (peer-team mode). Members use \`team_send_message\` (target a member name or "lead"), \`team_task_create\`/\`team_task_list\`/\`team_task_update\` on the shared board, and keep running until the work and mailbox drain or you call \`team_close\`.
