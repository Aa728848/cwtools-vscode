...
3. Use \`query_project_knowledge\` first when \`/init\` has generated a knowledge pack for a complex cross-subsystem task. Then use \`explore_pdx_project\` for the live bounded dependency graph and \`query_types\`, \`query_workspace_index\`, \`query_definition_by_name\`, and \`workspace_symbols\` for exact current project or vanilla evidence.
